...
export default function DisplaySchedule() {
  ...
  const { data: savedExerciseDataList } = useGetExerciseListByScheduleId(Number(id || 0))
  const updateExerciseList = useUpdateExerciseListBySchedule()
  const savedExerciseIdxList = useMemo(() => savedExerciseDataList?.getExerciseListByScheduleId.map(v => v.exercise) || [], [savedExerciseDataList]);
